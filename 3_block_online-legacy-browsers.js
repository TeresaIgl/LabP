...
let expName = '3_block_online';  // from the Builder filename that created this script
let expInfo = {
...
async function experimentInit() {
  ...
  text_4 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_4',
    text: 'Merci de votre participation.\napuyez sur espace pour mettre fin a cette session.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
function trials_2LoopBegin(trials_2LoopScheduler, snapshot) {
  return async function() {
    ...
    trials_2.forEach(function() {
      ...
      trials_2LoopScheduler.add(pauseRoutineBegin(snapshot));
      trials_2LoopScheduler.add(pauseRoutineEachFrame());
      trials_2LoopScheduler.add(pauseRoutineEnd(snapshot));
...
function colorRoutineEachFrame() {
  return async function () {
    ...
    Liste = ["red", "blue", "green", "yellow"];
    rcolor = Math.random.choice(Liste);
...
function stroopRoutineBegin(snapshot) {
  return async function () {
    ...
    // update component parameters for each repeat
    text_2.setColor(new util.Color(rcolor));
